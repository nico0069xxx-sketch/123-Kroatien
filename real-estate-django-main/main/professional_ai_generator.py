...
# Professional type translations
PROFESSIONAL_TYPE_NAMES = {
    'real_estate_agent': {
        'ge': 'Immobilienmakler',
        'en': 'Real Estate Agent',
        'hr': 'Agent za nekretnine',
    },
    'construction_company': {
        'ge': 'Bauunternehmen',
        'en': 'Construction Company',
        'hr': 'Građevinska tvrtka',
    },
    'lawyer': {
        'ge': 'Rechtsanwalt',
        'en': 'Lawyer',
...
async def _generate_professional_content_async(professional_context, language_code, language_name, professional_type, session_id):
    """
    Internal async function to generate professional content.
    """
    type_name = PROFESSIONAL_TYPE_NAMES.get(professional_type, {}).get('en', professional_type)
    
    prompt = f"""Generate comprehensive professional profile content in {language_name} for a {type_name} in Croatia.

Professional Information:
{professional_context}

Generate the following sections (each as a separate paragraph or list):
...
Format your response EXACTLY like this (keep the labels in English, content in {language_name}):
PROFILE_SUMMARY:
[content here]
...
Write ALL content in {language_name} only. Be professional, SEO-friendly, and engaging."""
    ...
    try:
        system_message = f"You are an expert content writer for professional service providers in the real estate and construction industry in Croatia. Write only in {language_name}."
...
def generate_professional_content(professional, language_code):
    """
    Generate professional content for a specific language.
    """
    import uuid
    language_name = LANGUAGES.get(language_code, 'English')
    
    # Build context from professional data
    prof_info = []
    ...
    try:
        ...
        try:
            result = loop.run_until_complete(
                _generate_professional_content_async(
                    professional_context, 
                    language_code, 
                    language_name, 
...
def generate_all_professional_content(professional):
    """
    ...
    for lang_code in LANGUAGES.keys():
        print(f"Generating content in {LANGUAGES[lang_code]}...")
        content = generate_professional_content(professional, lang_code)
        ...
        if content and content.get('profile_summary'):
            ...
            print(f"  ✓ {LANGUAGES[lang_code]} {action}")
        else:
